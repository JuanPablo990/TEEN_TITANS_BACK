...
class HistorialMementoTest {

    @Test
    void testGuardarEstadoHorario() {
        MateriaInscrita m = new MateriaInscrita("MAT101", "Matemáticas", 3);
        HistorialMemento memento = new HistorialMemento(List.of(m));

        assertEquals(1, memento.getEstadoHorario().size());
        assertEquals("MAT101", memento.getEstadoHorario().get(0).getCodigo());
    }
}
